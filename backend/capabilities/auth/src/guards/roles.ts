 ...
 * @example
 * ```ts
 * import { roleGuard, clientGuard, memberGuard } from '@oxlayer/capabilities-auth';
 *
 * // Custom role guard
 ...
 * @example
 * ```ts
 * import { memberGuard } from '@oxlayer/capabilities-auth';
 *
 * app.use('/api/app', memberGuard());
 * ```
 */
export function memberGuard(): MiddlewareHandler {
  return roleGuard({
    roles: ['member'],
  });
}
 ...
 *
 * For members and managers, this enforces hard workspace isolation
