...
export const BLUEPRINTS: Record<string, BlueprintConfig> = {
  ...
  'client': {
    ...
    defaultRoles: [
      ...
      { name: 'candidate', description: 'End user' },
    ],
    defaultMappers: [
...
export interface TenantContext {
  ...
  /** User type (candidate/admin_user) */
  userType?: string;
}
...
export function extractTenantContext(payload: any): TenantContext {
  ...
  // Extract organization (optional for candidates)
...
- [ ] Create role guards (platform, client, candidate)
...
- [ ] Update app/ for candidate portal
...
- [ ] Candidate accessing own workspace only
