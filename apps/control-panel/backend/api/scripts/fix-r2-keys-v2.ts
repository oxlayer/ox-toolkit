/**
 * Fix R2 keys - remove oxlayer-sdk/ prefix (bucket name is already in URL)
 */
...
async function fixR2Keys() {
  ...
  // Remove the oxlayer-sdk/ prefix from r2_key
  const result = await db.execute(sql`
    UPDATE package_releases
    SET r2_key = SUBSTRING(r2_key FROM 12)
    WHERE version = '2026_02_14_001'
    AND r2_key LIKE 'oxlayer-sdk/%'
