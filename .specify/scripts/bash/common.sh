...
" 2>/dev/null); then
                if [ -n "$sorted_presets" ]; then
                    ...
                    while IFS= read -r preset_id; do
                        local member="$presets_dir/$preset_id/templates/${template_name}.md"
                        [ -f "$member" ] && echo "$member" && return 0
                    done <<< "$sorted_presets"
                fi
                ...
                for preset in "$presets_dir"/*/; do
                    [ -d "$preset" ] || continue
                    local member="$preset/templates/${template_name}.md"
                    [ -f "$member" ] && echo "$member" && return 0
                done
            fi
        else
    ...
    if [ -d "$ext_dir" ]; then
        for ext in "$ext_dir"/*/; do
            ...
            local member="$ext/templates/${template_name}.md"
            [ -f "$member" ] && echo "$member" && return 0
        done
    fi
