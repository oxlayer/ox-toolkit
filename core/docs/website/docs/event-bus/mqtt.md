...
sidebar_label: MQTT
description: MQTT adapter for lightweight pub/sub messaging compatible with all MQTT 3.1.1/5.0 brokers
---
...
##### `onEnvelope<T>(eventType: string, handler: (envelope: EventEnvelope<T>) => Promise<() => Promise<void>>`

Subscribe to event envelopes.

##### `onAll<T>(handler: (event: T, envelope: EventEnvelope<T>) => Promise<() => Promise<void>>`
