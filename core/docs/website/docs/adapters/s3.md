...
title: S3 Adapter
sidebar_label: S3
description: S3 adapter for AWS S3 and S3-compatible services with upload, download, and presigned URL generation
---

# @oxlayer/capabilities-adapters-s3

S3 adapter for AWS S3 and S3-compatible services (MinIO, R2, etc.). Provides a simplified interface for object storage with upload, download, and presigned URL generation.
...
```bash
bun add @oxlayer/capabilities-adapters-s3
```
...
```typescript
import { createDefaultS3Client } from '@oxlayer/capabilities-adapters-s3';
