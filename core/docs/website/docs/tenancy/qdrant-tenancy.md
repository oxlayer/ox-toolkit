---
title: Qdrant Tenancy Adapter
sidebar_label: Qdrant Tenancy
description: Qdrant multi-tenant vector search adapter for tenant-isolated embeddings
---

# @oxlayer/capabilities-adapters-qdrant-tenancy
...
```bash
bun add @oxlayer/capabilities-adapters-qdrant-tenancy
```

## Dependencies

```bash
bun add @oxlayer/capabilities-tenancy @oxlayer/capabilities-vector
```
...
```typescript
import { createTenancyAwareQdrant } from '@oxlayer/capabilities-adapters-qdrant-tenancy';
