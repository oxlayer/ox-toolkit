---
title: ClickHouse Tenancy Adapter
sidebar_label: ClickHouse Tenancy
description: ClickHouse multi-tenant analytics adapter for tenant-isolated analytics
---

# @oxlayer/capabilities-adapters-clickhouse-tenancy
...
```bash
bun add @oxlayer/capabilities-adapters-clickhouse-tenancy
```

## Dependencies

```bash
bun add @oxlayer/capabilities-tenancy @oxlayer/capabilities-adapters-bitwarden-secrets
```
...
```typescript
import { createTenancyAwareClickHouse } from '@oxlayer/capabilities-adapters-clickhouse-tenancy';
