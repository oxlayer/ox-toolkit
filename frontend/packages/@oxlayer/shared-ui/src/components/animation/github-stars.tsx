...
export type GithubStarsNumberProps = Omit<SlidingNumberProps, "number" | "fromNumber">;

export function GithubStarsNumber({ padStart = true, ...props }: GithubStarsNumberProps) {
...
export type GithubStarsIconProps<T extends React.ElementType> = {
  icon: React.ComponentType<T> | React.ReactElement<T>;
  color?: string;
  activeClassName?: string;
} & React.ComponentProps<T>;

export function GithubStarsIcon<T extends React.ElementType>({
  icon,
  color = "currentColor",
  activeClassName,
  className,
  ...
  return (
    <div style={{ position: "relative" }}>
      {React.createElement(IconComponent, {
        "aria-hidden": "true",
        className: cn(iconProps.className as string, className),
      ...
      {React.createElement(IconComponent, {
        ...
        className: cn(iconProps.className as string, className, activeClassName),
        ...(iconProps as Record<string, unknown>),
        ...props,
      } as React.ComponentProps<T>)}
    </div>
...
export interface GithubStarsParticlesProps extends ParticlesEffectProps {
  children: React.ReactElement;
  size?: number;
}

export function GithubStarsParticles({
  children,
  size = 4,
  style,
  ...props
...
export interface GithubStarsLogoProps extends React.SVGProps<SVGSVGElement> {}

export function GithubStarsLogo(props: GithubStarsLogoProps) {
  return (
