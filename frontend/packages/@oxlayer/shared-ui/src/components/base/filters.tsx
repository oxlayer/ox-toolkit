...
const flattenFields = <T = unknown>(fields: FilterFieldsConfig<T>): FilterFieldConfig<T>[] => {
  ...
  for (const item of fields) {
    if (isFieldGroup(item) || isGroupLevelField(item as FilterFieldConfig<T>)) {
      result.push(...(item.fields ?? []));
    } else {
      result.push(item as FilterFieldConfig<T>);
    }
  }
  return result;
};
...
type FilterOperatorMap = {
  ...
  date: FilterOperator[];
  daterange: FilterOperator[];
  text: FilterOperator[];
  number: FilterOperator[];
  numberrange: FilterOperator[];
...
const createOperatorsFromI18n = (i18n: FilterI18nConfig): FilterOperatorMap => ({
  select: [
...
export const DEFAULT_OPERATORS: FilterOperatorMap = createOperatorsFromI18n(DEFAULT_I18N);
...
const getOperatorsForField = <T = unknown>(
  ...
  const operatorKey: keyof FilterOperatorMap =
    fieldType === "custom" || fieldType === "separator" ? "select" : fieldType;
...
function FilterOperatorDropdown<T = unknown>({
  ...
  return (
    <Menu>
      ...
      <MenuContent align="start" className="w-fit min-w-fit">
        {operators.map(op => (
          <MenuItem
            key={op.value}
          ...
          {field.customValueRenderer ? (
            ...
            <>
              ...
              {selectedOptions.length === 1
                ? (selectedOptions[0]?.label ?? context.i18n.select)
                : selectedOptions.length > 1
      ...
      return (
        <div className="flex items-center" data-slot="filters-item">
          <FilterInput
            ...
            field={field}
            {...(field.onInputChange !== undefined ? { onInputChange: field.onInputChange } : {})}
  ...
  if (["email", "url", "tel"].includes(field.type || "")) {
    ...
    return (
      <FilterInput
        ...
        {...(() => {
          const pattern = field.pattern ?? getPattern();
...
export function Filters<T = unknown>({
  ...
  if (addButtonText !== undefined) contextValue.addButtonText = addButtonText;
  if (addButtonIcon !== undefined) contextValue.addButtonIcon = addButtonIcon;
  if (addButtonClassName !== undefined) contextValue.addButtonClassName = addButtonClassName;
  if (addButton !== undefined) contextValue.addButton = addButton;
  if (trigger !== undefined) contextValue.trigger = trigger;
    ...
    <FilterContext.Provider value={contextValue}>
      <div className={cn(filtersContainerVariants({ variant, size }), className)}>
        {showAddButton && selectableFields.length > 0 && (
          ...
          >
            {addButton ? (
              <PopoverTrigger render={addButton as React.ReactElement<Record<string, unknown>>} />
            ) : (
              <PopoverTrigger
                render={
                  <button
