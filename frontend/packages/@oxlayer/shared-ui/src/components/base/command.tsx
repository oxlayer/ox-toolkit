  ...
  (
    ...
    return (
      <Dialog {...props}>
        ...
        >
          <Command>{children as React.ReactNode}</Command>
        </DialogContent>
      </Dialog>
    );
  }
...
function CommandInput({ className, ...props }: React.ComponentProps<typeof CommandBase.Input>) {
  return (
    <div data-slot="command-input-wrapper" className="flex h-9 items-center gap-2 border-b px-3">
      ...
      <CommandBase.Input
        ...
        className={cn(
          "placeholder:text-muted-foreground flex h-10 w-full rounded-md bg-transparent py-3 text-sm shadow-none! outline-hidden disabled:pointer-events-none disabled:opacity-50",
          className
        )}
        {...props}
      />
...
function CommandEmpty({ ...props }: React.ComponentProps<typeof CommandBase.Empty>) {
  return (
    <CommandBase.Empty data-slot="command-empty" className="py-6 text-center text-sm" {...props} />
  );
}
...
function CommandCheck({ className, ...props }: React.ComponentProps<"svg">) {
  return (
    <Check
      data-slot="command-check"
      className={cn("ml-auto size-4 shrink-0", className)}
      {...props}
    />
  );
}
