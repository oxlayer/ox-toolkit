 ...
 */
export type IntentActionType = 'create' | 'update' | 'delete' | 'custom';
 ...
 */
export interface UserIntent<T = unknown, P = Record<string, unknown>> {
 ...
 */
export interface WorkspaceData<T = Record<string, unknown>> {
  workspaceId: string;
  entities: T;
 ...
 */
export type EntitySyncStatus = DeliveryStatus;
 ...
 */
export interface WorkspaceExport<T = Record<string, unknown>, W = Record<string, unknown>> {
  version: string;
  exportedAt: string;
  workspace: W;
